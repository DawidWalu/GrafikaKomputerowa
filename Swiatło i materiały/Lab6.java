...
public class Lab6 extends GLJPanel implements GLEventListener  {
	...
	public void display(GLAutoDrawable drawable) {
		...
		Polygon_and_Triangle(gl2, 17, 5, 9);
		gl2.glPopMatrix();
	}
	...
	public void Podstawa(GL2 gl2, int n, int r) {
		...
		for (int i = 0; i <= n; i++) {
			gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT_AND_DIFFUSE, materials[2], 5);
	...
	public void Polygon_and_Triangle(GL2 gl2, int n, int r, int h) {
		...
		for (int i = 0; i <= n; i++) {
			gl2.glBegin(GL2.GL_POLYGON);
			gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT_AND_DIFFUSE, materials[2], 1);
			gl2.glVertex3d((double) (r * Math.cos(2 * i * Math.PI / n)), 1,
					(double) (r * Math.sin(2 * i * Math.PI / n)));
			gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT_AND_DIFFUSE, materials[2], 5);
			gl2.glVertex3d(0, h, 0);
			i++;
			gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT_AND_DIFFUSE, materials[2], 9);
			gl2.glVertex3d((double) (r * Math.cos(2 * i * Math.PI / n)), 1,
					(double) (r * Math.sin(2 * i * Math.PI / n)));
			i--;
			gl2.glEnd();
